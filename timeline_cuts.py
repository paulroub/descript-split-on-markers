...
if len(sys.argv) != 1:
    print("Usage: python timeline_cuts.py <timeline_file>")
    sys.exit(1)

timeline_fn = sys.argv[1]
dest_dir = os.path.dirname(timeline_fn)

if not os.path.isfile(timeline_fn):
    print(f"Error: Timeline file {timeline_fn} does not exist.")
    sys.exit(1)

clips = timeline.get_clips(timeline_fn)
...
for clip in clips:
    ...
    for part in clip["parts"]:
        ...
        part_fn = f"part_{part_count}.mp4"
